...
def main():
    ...
    for name in SPECIES:
        ...
        output_path = f"{DATA_DIR}/{name}_malat1_complement_chunks.csv"
        reversed_output_path = (
            f"{DATA_DIR}/{name}_malat1_reversed_complement_chunks.csv"
        )
        ...
        chunks, positions = chunk_sequence(seq, WINDOW_SIZE, STEP_SIZE)
        pd.DataFrame({"exon": chunks}).to_csv(reversed_output_path, index=False)
